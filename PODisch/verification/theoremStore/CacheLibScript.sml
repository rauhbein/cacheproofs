...
open wordsTheory wordsLib wordsSyntax bitstringTheory pairTheory pairSyntax;
...
val CacheClean_pmEqpm'_diffMsb_thm = Q.store_thm("CacheClean_pmEqpm'_diffMsb_thm",
`!(va:word64) (pa:word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) va' pa' (state:cache_state).
    (let (i, t, wi)    = lineSpec(va, pa) state               in
     let (i', t', wi') = lineSpec(va', pa') state             in
     let ns = w2n (word_log2 (state.DC.ccsidr.NumSets + 1w))  in
...
val CacheInvalidate_pmEqpm'_diffMsb_thm = Q.store_thm("CacheInvalidate_pmEqpm'_diffMsb_thm",
`!(va:word64) (pa:word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) va' pa' (state:cache_state).
    (let (i, t, wi)    = lineSpec(va, pa) state               in
     let (i', t', wi') = lineSpec(va', pa') state             in
     let ns = w2n (word_log2 (state.DC.ccsidr.NumSets + 1w))  in
...
val CacheRead_vaEQva'_thm = Q.store_thm("CacheRead_vaEQva'_thm",
...
val CacheRead_KeepHit_if_PaHitDc_thm = Q.store_thm("CacheRead_KeepHit_if_PaHitDc_thm",
 `!(va:word64) (pa:word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) (va':word64) (pa':word48) state.
...
val Fill_KeepHit_if_iNOTEQi'_thm = Q.store_thm("Fill_KeepHit_if_iNOTEQi'_thm",
 `!(va:word64) (pa:word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) (va':word64) (pa':word48) state.
  let (dc',pm')   = Fill (va, pa, pm, dc) state in
  let (i,t,wi)    = lineSpec(va, pa) state      in
  let (i',t',wi') = lineSpec(va', pa') state    in
...
val EPNotEqNONE_impEvict_thm = Q.store_thm("EPNotEqNONE_impEvict_thm",
  `!(va:word64) (pa:word48)  (pm:(word48->word8)) (dc:(48 word -> CSET))  (va':word64) (pa':word48) state.
  let (dc',pm')   = Fill (va, pa, pm, dc) state in
  let (i,t,wi)    = lineSpec(va, pa) state      in
  let (i',t',wi') = lineSpec(va', pa') state    in
...
val Fill_NotchgTag'_thm = Q.store_thm("Fill_NotchgTag'_thm",
...
val Fill_NotchgHitDC'Tag_thm = Q.store_thm("Fill_NotchgHitDC'Tag_thm",
...
val Fill_KeepHitDC'Tag_thm = Q.store_thm("Fill_KeepHitDC'Tag_thm",
 `!(va:word64) (pa:word48) (va':word64) (pa':word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) (x:word48) state.
  let (dc',pm')   = Fill (va, pa, pm, dc) state in
  let (i,t,wi)    = lineSpec(va, pa) state      in
  let (i',t',wi') = lineSpec(va', pa') state    in
...
val Fill_EpEqtag_tagDc'Miss_thm = Q.store_thm("Fill_EpEqtag_tagDc'Miss_thm",
...
val Fill_ifInLineRange_HitDC'Pa_thm = Q.store_thm("Fill_ifInLineRange_HitDC'Pa_thm",
 `!(va:word64) (pa:word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) (va':word64) (pa':word48) state.
  let (dc',pm')   = Fill (va, pa, pm, dc) state           in
  let (i, t, wi)  = lineSpec(va, pa) state                in
  let (i',t',wi') = lineSpec(va', pa') state              in
...
val linefill_slEq_diffInputDc_thm = Q.store_thm("linefill_slEq_diffInputDc_thm",
...
val linefill_slEq_diffInputDcAndalsoPM_thm  = Q.store_thm("linefill_slEq_diffInputDcAndalsoPM_thm",
...
val CacheRead_KeepMiss_if_PaMissDc_thm = Q.store_thm("CacheRead_KeepMiss_if_PaMissDc_thm",
 `!(va:word64) (pa:word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) (va':word64) (pa':word48) state.
  let (dc',pm')   = CacheRead (va, pa, pm, dc) state in
  let (i,t,wi)    = lineSpec(va, pa) state      in
  let (i',t',wi') = lineSpec(va', pa') state    in
...
val Fill_NotChgCache_ifEQSiAndalsoTag = Q.store_thm("Fill_NotChgCache_ifEQSiAndalsoTag",
...
val diffPa_imply_diffElement_thm = Q.store_thm("diffPa_imply_diffElement_thm",
 `!va pa va' pa' state.
  let (i, t, wi) = lineSpec(va, pa) state in
  let (i', t', wi') = lineSpec(va', pa') state in
   (invariant_cache) ==>
...
val Fill_causing_evict_EP_IsSome_thm = Q.store_thm("Fill_causing_evict_EP_IsSome_thm",
 `!(va:word64) (pa:word48) (va':word64) (pa':word48) (pm:(word48->word8)) (dc:(48 word -> CSET)) state.
  let (dc',pm')   = Fill (va, pa, pm, dc) state in
  let (i,t,wi)    = lineSpec(va, pa) state      in
  let (i',t',wi') = lineSpec(va', pa') state    in
...
val Fill_Keep_Miss_DiffTag_thm = Q.store_thm("Fill_Keep_Miss_DiffTag_thm",
  `!(va:word64) (pa:word48)  (pm:(word48->word8)) (dc:(48 word -> CSET))  (va':word64) (pa':word48) state.
   let (dc',pm')   = Fill (va, pa, pm, dc) state in
   let (i,t,wi)    = lineSpec(va, pa) state      in
   let (i',t',wi') = lineSpec(va', pa') state    in
