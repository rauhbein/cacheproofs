...
-- We do not need to save bi in history
-- The main reason we add bi to  history is to make sure that we would be able to
-- reconstruct physical address of line evicted  from cache when  it  needs to be
-- written back into the memory. Alternative solution is to use following function
-- to find the line associated with a given tag:

-- SI FindSl(t::TAG, dc::CACHE_ABS) =
-- {
--     var cache = dc;
...
--     for i in 0 .. itr - 1 do{
--         cset = cache([i]::SI).sl;
--         when (IsSome(cset(t))) do {
--                  idx <- i
--             }
--     };
--     return [idx div ([DC.ccsidr.Associativity]::nat + 1)]
-- }

-- Main point here is that this function works only when there si no allias in the 
...
-- CACHE_ABS * MEM_ABS WriteBackLine(li::SI, t::TAG,  pm::MEM_ABS, dc::CACHE_ABS, wi::nat)
-- measure ((2 ** [DC.ctr.DminLine]::nat) - wi) =
-- {
--     var PM = pm;
--     var cache = dc;    

--     if((2 ** [DC.ctr.DminLine]::nat) > wi)
--     then {(c, m) = WriteBack(li, t, wi, PM, cache);
--           cache <- c; PM <- m;
--           return WriteBackLine(li, t, PM, cache, wi + 1)
--     }
